...
```{r}
X_puntos <- puntos_maha |>
  select(z_disp, z_wt) |>
  as.matrix()

dist_euclidea <- as.matrix(stats::dist(X_puntos))
dist_mahalanobis <- as.matrix(
  proxy::dist(
    X_puntos,
    method = "Mahalanobis",
    cov = cov(Z_mtcars)
...
comparacion <- tibble(
  par = c("A-B", "C-D"),
  direccion = c("PC1", "PC2"),
  euclidea = c(dist_euclidea[1, 2], dist_euclidea[3, 4]),
  mahalanobis = c(dist_mahalanobis[1, 2], dist_mahalanobis[3, 4])
)
